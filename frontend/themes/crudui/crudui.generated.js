...
window.Vaadin.Flow['_vaadintheme_crudui_globalCss'] = window.Vaadin.Flow['_vaadintheme_crudui_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_crudui_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_crudui_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_crudui_globalCss'].push(target);
  }
  if (!document['_vaadintheme_crudui_componentCss']) {
    
    document['_vaadintheme_crudui_componentCss'] = true;
  }
